...
export default function LoginForm() {
    ...
    return (
      <Shell>
        <p className="text-sm text-white/60 text-center">
          ...
          <code className="text-white/80">supabase/README.md</code>) and restart.
        </p>
        <Link href="/" className="mt-4 block text-center text-sm text-blue-400 hover:text-blue-300">
          ← Back to the map
        </Link>
      </Shell>
    );
  ...
  return (
    <Shell>
      <h1 className="text-xl font-bold text-white text-center mb-1">{TITLES[mode]}</h1>
      ...
      <div className="mt-5 flex flex-col items-center gap-2 text-xs text-white/40">
        {mode === "signin" && (
          <>
            <button onClick={() => setMode("forgot")} className="hover:text-white/70">
        ...
        {(mode === "forgot" || mode === "update") && (
          <button onClick={() => setMode("signin")} className="hover:text-white/70">
            ← Back to sign in
          </button>
        )}
      </div>

      <Link href="/" className="mt-6 block text-center text-xs text-white/30 hover:text-white/60">
        ← Back to the map
      </Link>
    </Shell>
  );
